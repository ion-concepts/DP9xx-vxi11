...
class DP900(vxi11.Instrument):
    def __init__(self, host, *args, **kwargs):
        super(DP900, self).__init__(host, *args, **kwargs)
        idn = self.get_identification()
        match = re.match("Rigol Technologies,DP9\d\d", idn)
        if not match:
            msg = "Unknown device identification:",idn
            raise NameError(msg)
...
####################################################################################        
# Reference class for DP800. SOME COMMANDS DIFFERENT, DO NOT USE DIRECTLY FOR DP900
